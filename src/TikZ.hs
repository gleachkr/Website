...
renderSVG p@(Picture wrap attr tikz) = do
  ...
  if exists
    ...
    system "pdflatex tmp.tex 2>&1 > /dev/null"
    system "pdf2svg tmp.pdf tmp-1.svg 2>&1 > /dev/null"
...
writeTikzTmp wrap f tikz = do
  ...
  hPutStrLn h "\\nonstopmode"
  hPutStrLn h "\\documentclass[18pt, varwidth=4in]{standalone}"
  ...
  hPutStrLn h "\\usepackage{tikz-qtree}"
  hPutStrLn h "\\usepackage{bussproofs}"
  hPutStrLn h "\\usepackage{KMcalc}"
  hPutStrLn h "\\usepackage{arydshln}"
